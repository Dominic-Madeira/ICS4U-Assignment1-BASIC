...
const numMugwumps = 4
const maxTries = 10
const mugwumps: number[][] = new Array(numMugwumps).fill(0).map(() => [0, 0])
...
function playGame (): void {
  ...
  while (tries < maxTries) {
    ...
    xCoordinate = parseInt(xCoordinate.value)
    yCoordinate = parseInt(yCoordinate.value)
    ...
    for (let counter = 0; counter < numMugwumps; counter++) {
      ...
      if (mugwumps[counter][0] === xCoordinate && mugwumps[counter][1] === yCoordinate) {
        ...
        const distance = Math.sqrt(Math.pow(mugwumps[counter][0] - xCoordinate, 2) + Math.pow(mugwumps[counter][1] - yCoordinate, 2))
